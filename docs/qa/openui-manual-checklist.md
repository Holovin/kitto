...
- Each automatic repair request should append model-visible context that the previous draft was rejected, with a message like `Previous draft rejected due to: <codes>`, and the backend repair prompt should include that context before the corrected-source instruction. The repair prompt must include the full committed source while it is at or below `sourceMaxChars`, must not replace it with compact inventory/source summary context, and prompt I/O logs should expose `committedSourceIncluded`, `committedSourceChars`, `committedSourceProtected`, `repairPromptMaxChars`, and `droppedSections`.
