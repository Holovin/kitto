...
- frontend submit-time preflight must block requests whose `prompt`, `chatHistory[].content`, `currentSource`, `invalidDraft`, or serialized payload already exceeds the matching `GET /api/config` limit, show one clear builder error, and avoid sending that oversized request; the backend `413` limit remains the security boundary
