...
interface BuildAdditionalRulesOptions {
  structuredOutput?: boolean;
}

function buildIntentSpecificRules(intents: PromptIntentVector, structuredOutput: boolean) {
  return [
    ...SIMPLE_APP_RULES,
    ...(structuredOutput ? [] : ['Return only raw OpenUI Lang source. Do not wrap it in markdown, prose, or code fences.']),
    ...CORE_PROGRAM_RULES,
...
export function buildAdditionalRulesForPrompt(prompt: string | undefined, options: BuildAdditionalRulesOptions = {}) {
  ...
  return buildIntentSpecificRules(intents, structuredOutput);
}
